...
const Basketball = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:5000/api/auth/sportsItems/name/basketball`)
      .then((response) => {
  ...
  const handleUpdateScore = async (team) => {
    try {
      ...
      const response = await axios.put(`http://localhost:5000/api/auth/update-score_bask/${matchId}`, {
        team,
        score: newScore,
      });
  ...
  const handleAddMatch = () => {
    if (selectedMatch) {
      ...
      // Add new match
      axios
        .post("http://localhost:5000/api/auth/addMatch_bask", {
          teamA: newMatch.teamA,
          teamB: newMatch.teamB,
          name: `${newMatch.teamA || "Team A"} VS ${
            newMatch.teamB || "Team B"
